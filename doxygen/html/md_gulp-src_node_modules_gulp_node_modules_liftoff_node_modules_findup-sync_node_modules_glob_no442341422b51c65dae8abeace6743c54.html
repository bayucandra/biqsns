...
<title>biq-wp-sns: minimatch</title>
...
<script type="text/javascript">
$(document).ready(function(){initNavTree('md_gulp-src_node_modules_gulp_node_modules_liftoff_node_modules_findup-sync_node_modules_glob_no442341422b51c65dae8abeace6743c54.html','');});
</script>
<div id="doc-content">
...
<h2>Usage</h2>
<p>```javascript var minimatch = require("minimatch")</p>
<p>minimatch("bar.foo", "*.foo") // true! minimatch("bar.foo", "*.bar") // false! minimatch("bar.foo", "*.+(bar|foo)", { debug: true }) // true, and noisy! ```</p>
<h2>Features</h2>
...
<p>```javascript var Minimatch = require("minimatch").Minimatch var mm = new Minimatch(pattern, options) ```</p>
<h3>Properties</h3>
<ul>
...
<p>```javascript var isJS = minimatch(file, "*.js", { matchBase: true }) ```</p>
...
<p>```javascript var javascripts = fileList.filter(minimatch.filter("*.js", {matchBase: true})) ```</p>
...
<p>```javascript var javascripts = minimatch.match(fileList, "*.js", {matchBase: true})) ```</p>
