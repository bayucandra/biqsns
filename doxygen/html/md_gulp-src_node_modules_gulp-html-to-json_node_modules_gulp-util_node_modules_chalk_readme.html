...
<script type="text/javascript">
$(document).ready(function(){initNavTree('md_gulp-src_node_modules_gulp-html-to-json_node_modules_gulp-util_node_modules_chalk_readme.html','');});
</script>
<div id="doc-content">
...
<h2>Install</h2>
<p>``` $ npm install &ndash;save chalk ```</p>
<h2>Usage</h2>
<p>Chalk comes with an easy to use composable API where you just chain and nest the styles you want.</p>
<p>```js var chalk = require('chalk');</p>
<p>// style a string chalk.blue('Hello world!');</p>
<p>// combine styled and normal strings chalk.blue('Hello') + 'World' + chalk.red('!');</p>
<p>// compose multiple styles using the chainable API chalk.blue.bgRed.bold('Hello world!');</p>
<p>// pass in multiple arguments chalk.blue('Hello', 'World!', 'Foo', 'bar', 'biz', 'baz');</p>
<p>// nest styles chalk.red('Hello', chalk.underline.bgBlue('world') + '!');</p>
<p>// nest styles of the same type even (color, underline, background) chalk.green( 'I am a green line ' + chalk.blue.underline.bold('with a blue substring') + ' that becomes green again!' ); ```</p>
<p>Easily define your own themes.</p>
<p>```js var chalk = require('chalk'); var error = chalk.bold.red; console.log(error('Error!')); ```</p>
...
<p>```js var name = 'Sindre'; console.log(chalk.green('Hello s'), name); //=&gt; Hello Sindre ```</p>
<h2>API</h2>
...
<p>```js var ctx = new chalk.constructor({enabled: false}); ```</p>
...
<p>```js var chalk = require('chalk');</p>
<p>console.log(chalk.styles.red); //=&gt; {open: '[31m', close: '[39m'}</p>
<p>console.log(chalk.styles.red.open + 'Hello' + chalk.styles.red.close); ```</p>
...
<p>Example:</p>
<p>```js var chalk = require('chalk'); var styledString = getText();</p>
<p>if (!chalk.supportsColor) { styledString = chalk.stripColor(styledString); } ```</p>
<h2>Styles</h2>
<h3>Modifiers</h3>
<ul>
